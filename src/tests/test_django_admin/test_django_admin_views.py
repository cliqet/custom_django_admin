...
def test_get_model_fields_non_admin(api_client, non_admin_token):
    ...
    response = client.get(
        reverse('get_model_fields', kwargs={'app_label': 'django_admin', 'model_name': 'demomodel'}), 
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
    )
    assert response.status_code == 401
...
def test_get_model_fields_admin_not_found(api_client, limited_admin_token):
    ...
    response = client.get(
        reverse('get_model_fields', kwargs={'app_label': 'django_admin', 'model_name': 'nothing'}), 
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {limited_admin_token}'}
    )
...
def test_get_model_fields_edit_non_admin(api_client, non_admin_token, demo_model_instance):
    ...
    response = client.get(
        reverse(
            ...
            kwargs={'app_label': 'django_admin', 'model_name': 'demomodel', 'pk': demo_model_instance.pk}
        ), 
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
    )
...
def test_get_model_fields_edit_admin_not_found(api_client, superuser_token):
    ...
    response = client.get(
        reverse(
            'get_model_fields_edit', 
            kwargs={'app_label': 'django_admin', 'model_name': 'demomodel', 'pk': 1000}
        ), 
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {superuser_token}'}
    )
...
def test_get_model_listview_non_admin(api_client, non_admin_token):
    ...
    response = client.get(
        reverse(
            'get_model_listview', 
            kwargs={'app_label': 'django_admin', 'model_name': 'demomodel'}
        ),
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
    )
...
def test_get_inline_listview_non_admin(api_client, non_admin_token, type_instances):
    client = api_client()
    ...
    response = client.get(
        f"{reverse(
            'get_inline_listview', 
            kwargs={
                'parent_app_label': 'django_admin', 
                'parent_model_name': 'type', 
                'change_obj_pk': type_instances[0].pk
            }
        )}?inline_class=DemoModelCustomInline",
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
    )
    assert response.status_code == 401
...
def test_get_inline_listview_admin(api_client, superuser_token, type_instances):
    client = api_client()
...
def test_custom_action_delete_non_admin(api_client, non_admin_token):
    ...
    response = client.post(
        reverse(
            'custom_action_view', 
            kwargs={'app_label': 'django_admin', 'model_name': 'demomodel', 'func': CUSTOM_ACTIONS.DELETE_LISTVIEW}
        ),
        data=data,
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
...
def test_add_model_record_non_admin(api_client, non_admin_token):
    ...
    response = client.post(
        reverse(
            'add_model_record', 
            kwargs={'app_label': 'django_admin', 'model_name': 'country'}
        ),
        data=data,
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
...
def test_change_model_record_non_admin(api_client, non_admin_token, type_instances):
    ...
    response = client.post(
        reverse(
            'change_model_record', 
            kwargs={'app_label': 'django_admin', 'model_name': 'type', 'pk': type_instances[0].pk}
        ),
        data=data,
        format='json',
        **{'HTTP_AUTHORIZATION': f'Bearer {non_admin_token}'}
